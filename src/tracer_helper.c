...
#error struct bio has neither bi_bdev nor bi_partno.
	...
	if (likely(bio_sector_start >= dev->sd_sect_off &&
			   bio_sector_start < dev->sd_sect_off + dev->sd_size))
		return true;
...
bool tracer_is_bio_for_dev_only_queue(struct snap_device *dev, struct bio *bio)
{
	...
	return !test_bit(UNVERIFIED, &dev->sd_state) && tracer_queue_matches_bio(dev, bio) && active;
}

bool tracer_should_trace_bio(struct snap_device *dev, struct bio *bio)
{
	return dev && !bio_is_discard(bio) && bio_data_dir(bio) && bio_size(bio) &&
